...
def train_srresnet():
    ...
    plt.title("Triaining loss per epoch for SRResNet in Task B")
    plt.savefig('Triaining loss per epoch for SRResNet in Task B.png')

    plt.figure(2)
    plt.plot([i for i in range(len(Traning_PSNR_list))], Traning_PSNR_list)
    plt.ylabel("Triaining PSNR")
    plt.xlabel('Number of epochs')
    plt.title("Triaining PSNR per epoch for SRResNet in Task B")
    plt.savefig('Triaining PSNR per epoch for SRResNet in Task B.png')

    plt.figure(3)
    plt.plot([i*10 for i in range(len(valid_loss_list))], valid_loss_list)
    plt.ylabel("Validation loss")
    plt.xlabel('Number of epochs')
    plt.title("Validation loss per epoch for SRResNet in Task B")
    plt.savefig("Validation loss per epoch for SRResNet in Task B.png")

    plt.figure(4)
    plt.plot([i*10 for i in range(len(Validation_PSNR_list))], Validation_PSNR_list)
    plt.ylabel("Validation PSNR")
    plt.xlabel('Number of epochs')
    plt.title("Validation PSNR per epoch for SRResNet in Task B")
    plt.savefig("Validation PSNR per epoch for SRResNet in Task B.png")
