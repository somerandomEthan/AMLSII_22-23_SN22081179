...
# Model checkpoints
lr_dir = "./Datasets/DIV2K_train_LR_bicubic_X2/DIV2K_train_LR_bicubic/X2/0064x2.png"
hr_dir = "./Datasets/DIV2K_train_HR/0064.png" 
srgan_checkpoint = "./checkpoint_srgan_A.pth.tar"
srresnet_checkpoint = "./checkpoint_srresnet_A.pth.tar"
...
def visualize_sr(lr_img, hr_img, halve=False):
    ...
    lr_img = Image.open(lr_img, mode="r")
    lr_img = lr_img.convert('RGB')

    if halve:
        hr_img = hr_img.resize((int(hr_img.width / 2), int(hr_img.height / 2)),
                               Image.LANCZOS)
    lr_img_x = hr_img.resize((int(hr_img.width / 2), int(hr_img.height / 2)),
                           Image.BICUBIC)

    # Bicubic Upsampling
    bicubic_img = lr_img_x.resize((hr_img.width, hr_img.height), Image.BICUBIC)

    # Super-resolution (SR) with SRResNet
    sr_img_srresnet = srresnet(convert_image(lr_img, source='pil', target='[0, 1]').unsqueeze(0).to(device))
    sr_img_srresnet = sr_img_srresnet.squeeze(0).cpu().detach()
    sr_img_srresnet = convert_image(sr_img_srresnet, source='[-1, 1]', target='pil')

    # Super-resolution (SR) with SRGAN
    sr_img_srgan = srgan_generator(convert_image(lr_img, source='pil', target='[0, 1]').unsqueeze(0).to(device))
    ...
    # Display grid
    grid_img.save('Task_A.png')

    return grid_img


if __name__ == '__main__':
    grid_img = visualize_sr(lr_img=lr_dir, hr_img=hr_dir)
